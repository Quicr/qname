...
TEST_CASE("quicr::HexEndec 256bit Encode/Decode Test")
{
    ...
    const std::string mask = formatter_256bit.Encode(first_part, second_part, third_part, fourth_part, last_part);
    CHECK_EQ(mask, hex_value);

    const auto [one, two, three, four, last] = formatter_256bit.Decode(std::string_view(hex_value));
...
TEST_CASE("quicr::HexEndec Decode Throw Test")
{
    ...
    const std::string_view another_invalid_hex_value = "0x1111111111111111222222222222220000";
...
TEST_CASE("quicr::HexEndec Decode quicr::Name")
{
    ...
    CHECK_EQ(results, std::array<uint64_t, 3>{0x1111111111111111, 0x22222222222222, 0x33});
