...
class Namespace
{
    static constexpr Name mask(const Name& name, uint8_t sig_bits) { return name & ~(~0x0_name >> sig_bits); }
  ...
  public:
    ...
    constexpr bool contains(const Name& name) const { return mask(name, _sig_bits) == _name; }
    ...
    std::string to_hex() const { return static_cast<std::string>(_name) + "/" + std::to_string(_sig_bits); }
    ...
    friend std::ostream& operator<<(std::ostream& os, const Namespace& ns) { return os << ns.to_hex(); }
