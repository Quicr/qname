...
Name::Name(uint8_t *data, size_t length)
{
    if (length > sizeof(Name) * 2)
        throw NameException("Byte array length cannot be longer than length of Name: " +
...
Name::Name(const std::vector<uint8_t> &data)
{
    if (data.size() > sizeof(Name))
        throw NameException("Byte array length cannot be longer than length of Name: " + std::to_string(sizeof(Name)));
...
Name Name::operator>>(uint16_t value) const
{
    Name name(*this);
    name >>= value;
    return name;
...
Name Name::operator+(uint64_t value) const
{
    Name name(*this);
    name += value;
    return name;
...
void Name::operator+=(uint64_t value)
{
    if (_low + value < _low)
    {
        ++_hi;
...
Name Name::operator+(Name value) const
{
    Name name(*this);
    name += value;
    return name;
